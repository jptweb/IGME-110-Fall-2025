# Project 3 - Interactive Digital Prototype - Deliverable #4: *Peer Evaluation & Reflection*

### Overview

Reflect on your P3 experience and provide feedback to your teammates through an **individual Google Form survey**.

**Due: Wednesday, November 20th - Before your class starts**
- Section 8: Before 8:00 AM
- Section 2: Before 2:00 PM

**Format:** Google Form (link will be emailed Monday, November 18th)

**Grading:** 10% of your P3 grade

**Time Required:** Minimum 30 minutes

---

### What's In The Survey

- Overall project reflection and your contributions
- Team member evaluations (contribution and collaboration ratings)
- Specific, constructive feedback for each teammate
- Optional confidential comments for instructor

---

### Important

**This is similar to the P1 peer evaluation process.**

- Be honest, specific, and constructive
- Peer evaluations DO impact individual grades
- Anonymized feedback will be shared with teammates
- Confidential section only visible to instructor

**For detailed guidance:** See [full instructions and examples](p3-peer-eval-reflection.md)

---

### Submission

**Where:** Google Form link (emailed Monday, November 18th)

**Late Policy:** 10% penalty per day late

**This is an individual submission** - take the full 30 minutes.

---

[**<-- Prev: Deliverable #3 - Final Prototype & Presentation**](p3-hifi-deliverable.md)
